...
function UploadLogo() 
{
    ...
    for ($i = 0; $i < 2; $i += 1)
    {
        if ($_FILES["LogoUpload". ($i+1)]["name"] != "")
        {
            $uploadOk = 1;
            $target_file = $target_directory . basename($_FILES["LogoUpload". ($i+1)]["name"]);
            $fileInfo = finfo_open(FILEINFO_MIME_TYPE);
            $checkFile = finfo_file($fileInfo, $_FILES["LogoUpload". ($i+1)]["tmp_name"]);
            if ($checkFile === "image/gif" || $checkFile === "image/png" || $checkFile === "image/jpg" || $checkFile === "image/jpeg")
            {
                ...
                if ($_FILES["LogoUpload". ($i+1)]["size"] > 500000)
                {
            ...
            else
            {
                ...
                if ($statement = mysqli_prepare($link, $query))
                {
                    ...
                    $parameter_originalName = basename($_FILES["LogoUpload". ($i+1)]["name"]);
                    if (mysqli_stmt_execute($statement))
                    {
                        ...
                        if (mysqli_stmt_num_rows($statement) >= 1)
                        {
                            ...
                            if (mysqli_stmt_bind_result($statement, $resultTempName, $resultOriginalName))
                            {
                                ...
                                {  
                                    ...
                                    if ($i == 0)
                                    {
                                        $SurveyStructureObject->ClientLogo = $target_directory . $resultTempName;
                                    }
                                    else
                                    {
                                        $SurveyStructureObject->CompanyLogo = $target_directory . $resultTempName;;
                                    }
                                    ...
                                    $jsonData = file_get_contents($surveyJSONFile);
                                    $SurveyStructureObject = json_decode($jsonData);
                                    echo "RESULT: " . $SurveyStructureObject->CompanyLogo;
                                }
                            }
                        }
                        else
                        {
                            ...
                            if (move_uploaded_file($_FILES["LogoUpload". ($i+1)]["tmp_name"], $target_file))
                            {
                                ...
                                if ($statement = mysqli_prepare($link, $query))
                                {
                                    ...
                                    if (mysqli_stmt_execute($statement))
                                    {
                                        ...
                                        if ($i == 0)
                                        {
                                            $SurveyStructureObject->ClientLogo = $target_file;
                                        }
                                        else
                                        {
                                            $SurveyStructureObject->CompanyLogo = $target_file;
                                        }
